...
# Convert these to a dataframe using gen_gape_frame
...
for gape_frame in gape_frame_list:
    ...
    gape_frame['segment_center'] = [
        np.mean(x)/1000 for x in gape_frame.segment_bounds]

# Save gape_frame with both raw and adjusted bounds
...
# If not, remove the segment from gape_frame
gape_frame_list = []
for ind, row in tqdm(merge_df.iterrows()):
    ...
    gape_frame = gape_frame.loc[gape_frame.scored]
...
all_data_frame['animal_num'] = [
    x.split('_')[0] for x in all_data_frame.basename.values]

fin_gape_frames = []
for i, (_, this_row) in enumerate(all_data_frame.iterrows()):
    this_frame = this_row.gape_frame_raw
    this_frame['session_ind'] = i
...
for i, this_frame in enumerate(pal_frames):
    this_frame['session_ind'] = i
    this_frame['animal_num'] = all_data_frame.animal_num.values[i]
    this_frame['basename'] = all_data_frame.basename.values[i]
