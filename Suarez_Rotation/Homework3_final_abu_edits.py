...
for condition in range(n_conditions): 
    ...
    fig.suptitle('Gapes per Trial per Tastant (Condition {})'.format(condition)) #Creates a title for the plots kind of not needed
    plt.show()
...
## Always remove variables which aren't used later to make it easy for the
## reader
######################################
...
# ============================================================= #
## Calculating firing rates from the get-go
...
######################################
# Can't have a taste dimension for every array and still have only Quinine data

## Process all data beforehand and then plot
## This kind of modularization of code is useful when you're prototyping
## You make sure than one step is completely done and error-free and then move
## on to the next
######################################
...
counts_Hg_dQ = np.zeros((n_neurons, n_bins)) 
counts_Lg_dQ = np.zeros((n_neurons, n_bins))
counts_Hg_cQ = np.zeros((n_neurons, n_bins))
counts_Lg_cQ = np.zeros((n_neurons, n_bins))

counts_Hg_dQ_SEM = np.zeros((n_neurons, n_bins)) 
counts_Lg_dQ_SEM = np.zeros((n_neurons, n_bins))
counts_Hg_cQ_SEM = np.zeros((n_neurons, n_bins))
counts_Lg_cQ_SEM = np.zeros((n_neurons, n_bins))

######################################
# Sum the last axis because it's the same
laser_trials = np.sum(on_laser, axis= -1) > 0
######################################
...
for neuron in range(n_neurons): #Starts for loop to go through all the neurons (16 of them)
    ...
    ######################################
    # laser_trials is not separated by condition, each column in this case
    # gives the same information
    # And can't index by taste anymore
    ######################################
    #laser_trials = np.squeeze(np.array(np.where(on_laser[taste,:,condition]==1))) 

    ######################################
    # Taste dim not needed since we're only doing 2 tastes
    cond_trials = spike_array[:,:,neuron,:]
    ######################################
    
    ######################################
    ## We want to select trials where the laser is OFF
    ######################################
    ...
    ######################################
    # Since we pulled out trials with laser on in "laser_trials"
    # We used ~laser_trials to find trials where the laser is off
    ######################################

    ######################################
    ## Code below is largely the same, just updated for new indexing
    ######################################

    # Process dilute quinine
    dQ_laser_off_trials = np.where(~laser_trials[2])[0] 

    counts_Hg_dQ[neuron] = \
    np.asarray([np.sum(cond_trials[2,\
                dQ_laser_off_trials[Hg_dQ],\
                i*step_size:i*step_size+t_bin])/len(laser_trials) \
                         for i in range(n_bins)])*1000/t_bin

    counts_Hg_dQ_SEM[neuron] =\
    np.asarray(([np.std(
        np.sum(cond_trials[2,\
                dQ_laser_off_trials[Hg_dQ],\
                i*step_size:i*step_size+t_bin],axis=-1))\
                     for i in range(n_bins)])/np.sqrt(laser_trials.shape[0]))*1000/t_bin

    counts_Lg_dQ[neuron] = \
    np.asarray([np.sum(cond_trials[2,\
                dQ_laser_off_trials[Lg_dQ],\
                i*step_size:i*step_size+t_bin])/len(laser_trials) \
                         for i in range(n_bins)])*1000/t_bin

    counts_Lg_dQ_SEM[neuron] =\
    np.asarray(([np.std(
        np.sum(cond_trials[2,\
                dQ_laser_off_trials[Lg_dQ],\
                i*step_size:i*step_size+t_bin],axis=-1))\
    ...
    # Process concentrated quinine
    cQ_laser_off_trials = np.where(~laser_trials[3])[0] 

    counts_Hg_cQ[neuron] = \
    np.asarray([np.sum(cond_trials[3,\
                cQ_laser_off_trials[Hg_cQ],\
                i*step_size:i*step_size+t_bin])/len(laser_trials) \
                         for i in range(n_bins)])*1000/t_bin

    counts_Hg_cQ_SEM[neuron] =\
    np.asarray(([np.std(
        np.sum(cond_trials[3,\
                cQ_laser_off_trials[Hg_cQ],\
                i*step_size:i*step_size+t_bin],axis=-1))\
                     for i in range(n_bins)])/np.sqrt(laser_trials.shape[0]))*1000/t_bin
    ...
    counts_Lg_cQ[neuron] = \
    np.asarray([np.sum(cond_trials[3,\
                cQ_laser_off_trials[Lg_cQ],\
                i*step_size:i*step_size+t_bin])/len(laser_trials) \
                         for i in range(n_bins)])*1000/t_bin

    counts_Lg_cQ_SEM[neuron] =\
    np.asarray(([np.std(
        np.sum(cond_trials[3,\
                cQ_laser_off_trials[Lg_cQ],\
                i*step_size:i*step_size+t_bin],axis=-1))\
...
for neuron in range(n_neurons): #Starts for loop to go through all the neurons (16 of them)
    ...
    ax.errorbar(bins, counts_Hg_dQ[neuron,  :], \
            yerr = counts_Hg_dQ_SEM[neuron],\
            label = 'High Gape',color='midnightblue')
    ax.errorbar(bins, counts_Lg_dQ[neuron,  :], \
            yerr = counts_Lg_dQ_SEM[neuron],\
    ...
    ax.errorbar(bins, counts_Hg_cQ[neuron,  :], \
            yerr = counts_Hg_cQ_SEM[neuron],label = 'High Gape',color='midnightblue')
    ax.errorbar(bins, counts_Lg_cQ[neuron,  :], \
            yerr = counts_Lg_cQ_SEM[neuron],\
            label = 'Low Gape',color='darkred')
    ax.legend()
