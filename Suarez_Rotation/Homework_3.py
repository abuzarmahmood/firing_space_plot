...
#Plotted 4 tastes as heatmaps for one conditions
    ...
    panel = pd.Panel(np.rollaxis(prob_matrix,2)).to_frame()
...
#Plot PSTH
    counts_Hg_dQ = np.zeros((n_neurons, n_tastes, n_bins)) #gives array of the same dimension but w/ 0 (dummy array to fill in)
    ...
    for neuron in range(n_neurons): #Starts for loop to go through all the neurons (16 of them)
        for taste in range(n_tastes): #Starts with the first taste at 0 until done
            ...
            np.asarray([np.sum(cond_trials[laser_trials[np.array(Hg_dQ)],i*step_size:i*step_size+t_bin]*1000/t_bin) \
                                 for i in range(n_bins)])
            counts_Lg_dQ[neuron, taste] = \
            np.asarray([np.sum(cond_trials[laser_trials[np.array(Lg_dQ)],i*step_size:i*step_size+t_bin]*1000/t_bin) \
                                 for i in range(n_bins)])
            counts_Hg_cQ[neuron, taste] = \
            np.asarray([np.sum(cond_trials[laser_trials[np.array(Hg_cQ)],i*step_size:i*step_size+t_bin]*1000/t_bin) \
                                 for i in range(n_bins)])
            counts_Lg_cQ[neuron, taste] = \
            np.asarray([np.sum(cond_trials[laser_trials[np.array(Lg_cQ)],i*step_size:i*step_size+t_bin]*1000/t_bin) \
                                 for i in range(n_bins)])
    
            counts_Hg_dQ_SEM = (([np.std(cond_trials[laser_trials[np.array(Hg_dQ)],i*step_size:i*step_size+t_bin]*1000/t_bin) \
                             for i in range(n_bins)])/np.sqrt(laser_trials.shape[0]))
            counts_Lg_dQ_SEM = (([np.std(cond_trials[laser_trials[np.array(Lg_dQ)],i*step_size:i*step_size+t_bin]*1000/t_bin) \
                             for i in range(n_bins)])/np.sqrt(laser_trials.shape[0]))
            counts_Hg_cQ_SEM = (([np.std(cond_trials[laser_trials[np.array(Hg_cQ)],i*step_size:i*step_size+t_bin]*1000/t_bin) \
                             for i in range(n_bins)])/np.sqrt(laser_trials.shape[0]))
            counts_Lg_cQ_SEM = (([np.std(cond_trials[laser_trials[np.array(Lg_cQ)],i*step_size:i*step_size+t_bin]*1000/t_bin) \
                             for i in range(n_bins)])/np.sqrt(laser_trials.shape[0]))
            ...
            if taste >= 2:
                ...
                ax.errorbar(bins, counts_Hg_cQ[neuron, taste, :], yerr = counts_Hg_dQ_SEM,label = 'High Gape',color='midnightblue')
