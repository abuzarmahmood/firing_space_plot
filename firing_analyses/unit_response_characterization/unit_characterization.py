...
def isi_rate(array, smoothing_fraction = 0.05):
    ...
    filt_isi = [np.convolve(x, np.ones(this_len), mode = 'same')/this_len\
...
dir_list = sorted([x.strip() for x in open(dir_list_path,'r').readlines()])
...
for data_dir in dir_list:
    ...
    region_unit_counts = [len(x) for x in dat.region_units]
    count_dict = dict(zip(dat.region_names, region_unit_counts))
    print(f'{dat.hdf5_name} : {count_dict}')

    group_names = dat.car_names
    group_units = dat.car_units

    region_frame = pd.concat(\
            [pd.DataFrame({'region' : [region]*(len(nrns)),
                            'neuron' : nrns}) \
                    for region, nrns in zip(group_names, group_units)])
    ...
    conv_rate = dat._calc_conv_rates(25,250,1,psths)

    zscore_isi_rate = stats.zscore(firing_array,axis=-1)
    zscore_conv_rate = stats.zscore(conv_rate,axis=-1)

    conv_x = np.linspace(0,7000, conv_rate.shape[-1])
    zscore_isi_long = np.reshape(zscore_isi_rate, (-1, zscore_isi_rate.shape[-1]))
    zscore_conv_long = np.reshape(zscore_conv_rate, (-1, zscore_conv_rate.shape[-1]))

    fig,ax = visualize.gen_square_subplots(zscore_conv_long.shape[0])
    for this_isi, this_conv, this_ax in zip(zscore_isi_long, zscore_conv_long, ax.flatten()):
        this_ax.plot(this_isi)
        this_ax.plot(conv_x, this_conv)
    plt.show()

    #visualize.firing_overview(stats.zscore(firing_array.swapaxes(0,1),axis=-1), cmap = 'viridis')
    ##visualize.firing_overview(firing_array.swapaxes(0,1))
    #visualize.firing_overview(stats.zscore(conv_rate.swapaxes(0,1),axis=-1), cmap = 'viridis')
    ##visualize.firing_overview(conv_rate.swapaxes(0,1))
    #plt.show()
    ...
    #                       
    ########################################
    plot_super_dir = '/media/bigdata/Abuzar_Data/unit_response_plots'
    plot_dir = os.path.join(plot_super_dir, "/".join(dat.data_dir.split('/')[-3:]))
    if plot_bool: 
        ...
        g = sns.relplot(x='duration', y='time_on', 
                col='region',data = mean_nrn_frame)
        plt.subplots_adjust(0.1,0.1, top = 0.8)
        plt.suptitle(f'{dat.hdf5_name}' + '\n' 'Mean responses')
        plt.xlim([0,post_upper_lim])
        plt.ylim([0,post_upper_lim])
        ...
        region_label_data = list(nrn_ind_frame.groupby('region')) 
        region_sig_array = [sig_conv[x[1].response] for x in region_label_data]
        mean_sig_array = [np.mean(x,axis=0) for x in region_sig_array]
        # Get where to put neuron markers
        region_response_counts = [x[1].groupby('neuron').count() for x in region_label_data]
        region_response_markers = [np.cumsum(x.response) for x in region_response_counts]
        region_response_markers = [x- 0.5 for x in region_response_markers]

        fig,ax = plt.subplots(2,len(region_sig_array), sharex = 'col', figsize = (10,10))
        if len(region_sig_array) < 2:
            ax = ax[:,np.newaxis]
        for num in range(len(region_sig_array)):
            ax[0,num].imshow(region_sig_array[num],aspect='auto')
            ax[0,num].hlines(region_response_markers[num],0,
                    region_sig_array[0].shape[-1], 
                    color = 'blue', linestyles = 'dashed', linewidth = 2)
            ax[1,num].plot(mean_sig_array[num])
            ax[0,num].set_title(region_label_data[num][0])
            ax[1,num].set_ylim([0,1])
        #plt.show()
        plt.suptitle(f'{dat.hdf5_name}' + '\n' 'Region responses')
        plt.subplots_adjust(0.1,0.1, top = 0.9)
        fig.savefig(os.path.join(plot_dir,'all_region_responses'))
        ...
        for num in range(len(firing_long)):
            ...
            this_ax.axhline(\
                    pre_stim_ci[num][0], 0, len(zscore_y), 
                    alpha = 0.5, color ='red')
            this_ax.axhline(\
                    pre_stim_ci[num][1], 0, len(zscore_y), 
                    alpha = 0.5, color ='red')
        for num, up, down in zip(response_num, first_up, first_down):
            ax.flatten()[num].\
                    axvspan(up + stim_t ,down + stim_t, 
    ...
    epoch_stim_rates = \
            np.array([np.mean(post_stim_psths[...,lim[0]:lim[1]],axis=-1)\
    ...
    dist_list = \
            [distmat(x[:,np.newaxis],x[:,np.newaxis]) \
            for x in epoch_stim_rates_long] 
    ...
    epoch_neuron, grouped_dat = \
            zip(*list(mean_dists_frame.groupby(['epoch','neuron'])))
    ...
    mean_dists_array = np.array([x[1].dist \
            for x in mean_dists_frame.groupby('neuron')])
    ...
    diff_region_list = [x[1].region.iloc[0] \
            for x in diff_dists_frame.groupby('neuron')]
    ...
    if len(group_names) > 1 and len(region_encoding) > 1:
        ...
        if len(region_inds) < 2:
            ...
            if plot_bool:
                sns.clustermap(mean_diff_dists_array[region_inds],
                    row_colors = np.array(row_colors)[region_inds], 
                    col_cluster=False)
                plt.subplots_adjust(0.1,0.1, top = 0.9)
                ...
                fig.savefig(\
                        os.path.join(plot_dir,
                            f'raw_dists_{"_".join(region_names)}'))
                #plt.show()
   ...
   #################################################################################
    ...
    dat.firing_rate_params = dat.default_firing_params
    dat.firing_rate_params['step_size'] = 250
    ...
    for this_ind in inds:
        this_dat = firing_array[tuple(this_ind)]
        # Add random noise to avoid 0 errors
        this_dat += np.random.random(this_dat.shape)*1e-3
        p_val_array[tuple(this_ind)] = kruskal(*this_dat)[1]

    stim_t = 2000
    t_vec = np.arange(0,spikes.shape[-1],
            dat.firing_rate_params['step_size']) - stim_t
    ...
    with tables.open_file(dat.hdf5_path,'r+') as h5:
        if fin_save_path not in h5:
            ...
            remove_node(fin_save_path, h5, recursive = True)
            h5.create_group(save_path, group_name)
    ...
    sorted_sig_arrays = [cut_p_val_sig_array[units] for units in group_units]

    fig,ax = plt.subplots(2,len(group_names), sharex=True)
    if len(group_names) < 2:
        ax = ax[:,np.newaxis]
    plot_order = np.argsort(group_names)
    for num,this_array in enumerate(sorted_sig_arrays):
        ax[0,plot_order[num]].pcolormesh(
                t_vec[wanted_inds], np.arange(this_array.shape[0]), this_array)
        ax[1,plot_order[num]].plot(t_vec[wanted_inds], np.mean(this_array,axis=0))
        ax[1,plot_order[num]].set_ylim([0,1])
        ax[0,plot_order[num]].set_title(group_names[num])
