...
# data_base_dir = '/home/exouser/Desktop/TG22' 
data_base_dir = '/media/storage/thomas_data/TG22'

data_dir_list = glob.glob(os.path.join(data_base_dir, '*'))
states_vec = np.arange(2,9) 
region_vec = ['gc','pc']
transform_vec = ['None','spike_shuffled']
taste_list = [*[int(x) for x in np.arange(6)], 'all'] 

param_combs = list(product(
        data_dir_list,
        states_vec,
        region_vec,
        transform_vec,
...
for this_param_set in tqdm(param_combs):
    (
        this_data_dir,
        this_states,
        this_region,
    ...
    print(this_param_set)

    # Specify params for fit
    model_parameters = dict(
        states= this_states, #4,
        fit=40000,
        samples=20000,
    ...
    preprocess_parameters = dict(
        time_lims=[2000, 4000],
        bin_width=50,
        data_transform= this_transform, #'None',  # Can also be 'spike_shuffled','trial_shuffled'
        )

    FitHandler_kwargs = dict(
        data_dir= this_data_dir, #'/path/to/data/directory',
        taste_num= this_taste, #0,  # Can also be 'all'
        region_name= this_region, #['CA1'],  # Should match specification in info file
        laser_type=None,
        experiment_name='TRG_GC_PC',
        )
    ...
    save_path = handler.database_handler.model_save_path + '.pkl'
