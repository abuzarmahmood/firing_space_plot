...
def parallelize(func, iterator, n_jobs=16):
    ...
    return Parallel(n_jobs=n_jobs, max_nbytes=1e6)(delayed(func)(this_iter) for this_iter in tqdm(enumerate(iterator)))
...
if reprocess_data:
    ...
    for ind, spikes in enumerate(spike_list):
        ...
        np.save(os.path.join(spike_list_path,
                f'{ind:03}_spike_inds.npy'), spike_inds_list[ind])
    ...
    # Load ind_frame
    ind_frame = pd.read_csv(os.path.join(
        save_path, 'ind_frame.csv'), index_col=0)
...
def process_ind(ind_num, this_ind):
    ...
    for i in trange(n_max_tries):
        if len(fit_list) < n_fits:
            try:
                res, temp_design_mat, = gt.gen_actual_fit_sparse(
                    hist_filter_len=hist_filter_len_bin,
                    stim_filter_len=stim_filter_len_bin,
                    coupling_filter_len=coupling_filter_len_bin,
                    basis_kwargs=basis_kwargs,
    ...
    if len(fit_list) > 0:
        ...
        p_val_frame.rename(
            columns={'index': 'param', 0: 'p_val'}, inplace=True)
        ...
        ll_frame['fit_num'] = np.repeat(
            np.arange(len(fit_list)), n_shuffles_per_fit)
        ...
        design_spikes = [x.sort_values(
            'trial_time').spikes.values for _, x in design_trials]
        design_spikes_array = np.stack(design_spikes)
        design_spikes_tuple = np.array(np.where(design_spikes))
        np.save(os.path.join(fin_save_path,
                f'{this_ind_str}_design_spikes.npy'), design_spikes_tuple)
        ...
        pred_spikes = [x.sort_values(
            'trial_time').spikes.values for _, x in pred_trials]
        pred_spikes_array = np.stack(pred_spikes).astype(np.float16)
        np.save(os.path.join(fin_save_path,
                f'{this_ind_str}_pred_spikes.npy'), design_spikes_tuple)
    else:
