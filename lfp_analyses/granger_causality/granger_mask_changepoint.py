...
wanted_inds = np.where(w_state_counts == 5)
...
for i in range(ax.shape[0]):
    ...
    ax[0,i].set_title(dir_names[i])
    ax[1,i].imshow(mean_wanted_obs[i], aspect='auto', origin='lower',
                   extent=[time_vec[0], time_vec[-1],
                           freq_vec[0], freq_vec[-1]],
                   interpolation='none')
    ax[1,i].set_yticks(yticks)
...
plt.tight_layout()
fig.savefig(os.path.join(plot_dir_base, 'granger_mask_changepoints_dirichlet_5state_ppc.png'),
            dpi = 300)
plt.close(fig)
