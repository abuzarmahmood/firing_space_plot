...
for dir_name in tqdm(dir_list):
    ...
    print(f'Processing PAC for {basename}')
...
for region_idx, region_name in enumerate(region_names):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(plot_dir, f'pac_comodulogram_{region_name}.png'), dpi=300)
    plt.close()
...
for region_idx, region_name in enumerate(region_names):
    ...
    plt.savefig(os.path.join(plot_dir, f'pac_comparison_{region_name}.png'), dpi=300)
    plt.close()

print("PAC plotting complete. Plots saved to plots directory.")
