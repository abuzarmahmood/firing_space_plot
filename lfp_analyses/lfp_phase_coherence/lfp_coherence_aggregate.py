...
for this_node_num in tqdm(range(len(phase_node_path_list))):
    ...
    with tables.open_file(data_hdf5_path,'r+') as hf5:
        ...
        hf5.create_array(node_path_list[this_node_num], 
                            'phase_difference_array', phase_diff)
        hf5.create_array(node_path_list[this_node_num], 
    ...
    amplitude_array_split = [amplitude_array[:,x] \
                for x in relative_channel_num_split]
    mean_channel_amplitude_array = np.array([np.mean(x,axis=1) \
                for x in amplitude_array_split])

    # Extract relevant channels
    min_err_chan_amp = [np.squeeze(amplitude_array[:,chan]) \
                for chan in min_err_channel_nums]
    min_err_spectrograms = [np.mean(dat, axis = (0,1)) \
                for dat in min_err_chan_amp]
    norm_spectrograms = [normalize_timeseries(dat,time_vec,2) \
                for dat in min_err_spectrograms]
