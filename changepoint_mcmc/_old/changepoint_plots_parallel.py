...
#model_path = '/media/bigdata/Abuzar_Data/AM39/AM39_4Tastes_210203_123522/saved_models/vi_4_states/simulate_vi_4states_40000fit_1500_4000time_50bin.pkl'
...
def add_plot(f,ax,x,scalar):
    for num in range(len(ax.flatten())):
        ax.flatten()[num].plot(x,np.sin(x*num*scalar))
    return f
x = np.linspace(0,10,1000)
y = np.sin(x)

f_list = [add_plot(*plt.subplots(2,2),x,num) for num in range(10)]
f_list[9].show()
...
def create_changepoint_plots(spike_array, stft_array, tau_samples, trial_inds_list, 
    ...
    trial_inds = full_trial_inds_list[fig_num]
    trial_count = len(trial_inds)

    fig, ax = plt.subplots(trial_count,3,#sharex='col', 
                        figsize = (20,trial_count*3))

    args_list = [(*plt.subplots(len(x),3),x) for x in trial_inds_list] 

    def this_single_plot(fig,ax,trial_inds):
        f = single_plot(fig, ax, plot_spikes, stft_cut, 
                tau_samples, trial_inds,
    ...
    f_list = [this_single_plot(*args) for args in tqdm(args_list)]

def save_fig(fig_num,fig):
    fig.savefig(os.path.join(\
            this_plot_dir,'taste{}_changepoints'.format(fig_num)),dpi=300)
    plt.close(fig_num)
                
    parallelize(save_fig, [(x,y) for x,y in enumerate(f_list)])

    import time
    start = time.time()
    f_list=parallelize(this_single_plot, args_list)
    end = time.time() - start
    print(end)
